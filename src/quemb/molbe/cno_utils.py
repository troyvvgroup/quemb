...
def get_cnos(TA, TA_vir, TA_occ, hcore_full, eri_full, nocc):
    ###################
    # Occupieds first #
    ###################
...
def FormPairDensity(Vs, mo_occs, mo_coeffs, mo_energys, Type):
    ...
    print("V, V.shape", V.shape, V)
    mo_energy_occ = mo_energys[:nOcc]
    mo_energy_vir = mo_energys[nOcc:]
    print("mo_energy_occ", mo_energy_occ)
    print("mo_energy_vir", mo_energy_vir)
    ...
    print("delta_T dim", delta_T.shape)
    print("delta_T dim", delta_T.shape)
    # Type = 0 or 1 for Occ or Vir
    if Type == 0:
        #P = -1 * np.einsum('ikab,kjab->ij', T, Tt)
...
def GetCASSAdditions(CASS, nfA, nbA, nfA_, nelecA, nocc):
    # Add occupied orbitals until requested # of electrons is achieved and then add on virtual orbitals until orbitals are satisfied
    '''
    There are many difference cases here that I will describe.
        * For any integers number of electrons or orbitals, this reads the same CAS normally, include that many orbitals
          and that many electrons, taking into account the orbitals and electrons already in the fragment and bath.
        * For an integer, negative number of electrons or orbitals, this means add on that many electrons and orbitals,
          not taking into account the electrons and orbitals already in the CASS from the fragment and bath.
        * For a float of electrons, the number of orbitals represents the proportion of fragment orbitals to add to the space.
          CASS[0] is then the proportion of total orbitals to occupied orbitals desired in this expanded space. If fewer
          electrons are desired than already in the space, then no occupied orbitals are added. We prioritize fixing the
          total orbitals over the ratio of orbitals to occupied orbitals.
        * For a float of orbitals, this means add that fractional number of virtual orbitals to the CASS based on the number
          of occupied orbitals. Hence we multiply this number by the number of bath orbitals (usually the number of occupied
          orbitals) and then subtract the number of fragment orbitals (usually the number of virtual orbitals). The remaider 
          is how many virtual orbitals are needed to get the ratio of occ:vir orbitals
